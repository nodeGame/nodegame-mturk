...
vorpal
    .command('createHIT', 'Creates a new HIT.')
    .option('-q, --question [q]',
            'Question template for the HIT')
    .option('-t, --title [t]',
            'The title of the HIT')
    .option('-d, --description [d]',
            'The description of the HIT')
    .option('-c, --completiontime [c]',
            'The time to complete the HIT (in secs)')
    .option('-l, --lifetime [l]',
            'The time the HIT will be available')
    .option('-a, --assignments [a]',
           'The number of assignments')
    .option('-r, --reward [r]',
            'The reward for the HIT')
    .option('-k, --token [k]',
            'The unique request token for the HIT')
    .action(function(args, cb) {
        let o = args.options;
        let opts = {};
        // Convert input parameters.
        opts.Question = o.question;
        opts.Title = o.title;
        opts.Description = o.description;
        opts.AssignmentDurationInSeconds = o.completiontime;
        opts.LifetimeInSeconds = o.lifetime;
        opts.MaxAssignments = o.assignments;
        opts.UniqueRequestToken = o.token;
        modules.create.createHIT(opts, (err) => cb());
    });
...
vorpal
    ...
    .option('-f, --fileMessage [file]')
    .option('-F, --fileWorkerIds [filename]')
    // .option('-c, --confirm')
    .action(function(args, cb) {
        let o = args.options;
        if (o.fileMessage) {

            if (o.Subject || o.MessageText) {
                logger.warn('option fileMessage is not allowed with ' +
                            'options Subject and MessageText');
                cb();
                return;
            }
            let res = modules.notify.loadMessageFileSync(o.fileMessage);
            if (!res) return cb();
            o.Subject = res.Subject;
            o.MessageText = res.MessageText;
        }
        if (o.fileWorkerIds) {
            if (o.WorkerIds) {
                logger.warn('options fileWorkerIds is not allowed with ' +
                            'option WorkerIds');
                cb();
                return;
            }
            let res = modules.notify.loadWorkerIdsFileSync(o.fileWorkerIds);
            if (!res) return cb();
            o.WorkerIds = res;
        }
        modules.notify.notify(o, (err) => cb());
    });
...
vorpal
    ...
    .option('-m --MaxHITsResults <n>',
            'max number of HITs returned from server (default=10)')
    .autocomplete([
    ...
    .action(function(args, cb) {
        if (args.what === 'HITId') {
            ...
            modules.get.HITs({
                ...
                MaxResults: args.options.MaxHITsResults
            }, cb);
        }
