...
function createInputCodesDb() {
    ...
    db.on('insert', function(code) {
        // TODO: check this: have we already considered case insensitive?
...
function createResultsDb() {
    ...
    db.on('insert', function(i) {
        ...
        ci = cfg.fieldsCaseInsentitive;

        // Check no duplicates first, then validate actual values.

        propertyValue = getPropertyValue(i, 'id', ci);
        ...
        propertyValue = getPropertyValue(i, 'workerId', ci);
        ...
        propertyValue = getPropertyValue(i, 'exitCode', ci);
        ...
        propertyValue = getPropertyValue(i, 'assignmentId', ci);
        ...
        // Enforce the existence of the field as in `fields`,
        // if case insensitive is requested.
        if (ci) {
            getPropertyValue(i, 'hitId', ci);
            getPropertyValue(i, 'bonus', ci);
            getPropertyValue(i, 'reason', ci);
            getPropertyValue(i, 'qualificationTypeId', ci);
            getPropertyValue(i, 'reject', ci);
            getPropertyValue(i, 'approve', ci);
            getPropertyValue(i, 'assignmentStatus', ci);
            getPropertyValue(i, 'requesterFeedback', ci);
        }
...
// Get the requested `fields` property potentially case-insensitive
//
// If case-insensitive is set, it also makes sure that the property
// exists in the object according to original name in `fields`.
function getPropertyValue(obj, prop, ci) {
    var res, propName;
    propName = cfg.fields[prop];
    if (obj.hasOwnProperty(propName)) {
        res = obj[propName];
    }
    else if (ci) {
        res = obj[cfg.fieldsI[prop]];
        // Enforce existence.
        obj[propName] = res;
    }
    return res;
