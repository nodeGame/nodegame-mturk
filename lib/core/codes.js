...
function loadResults(args, cb) {
    ...
    validateParams = {
        ...
        exitCodeField: cfg.fields.exitCode
    };
...
function createResultsDb() {
	...
	// TODO: In progress. If insensitive, we should
	// make sure that the object has one version of
	// the property written (as in fields), so we
	// do not always double check whcih one.

	// TODO: reorganize. Do we need validateCode?

	// TODO: some properties are hardcoded.
	
	// TODO: ID case insensitive?
	if (this.id.get(i.id)) {
	...
	if (validateLevel && 'string' !== typeof propertyValue) {
	    return 'workerId must be string. Found: ' + propertyValue;
	}
	...
	if (validateLevel && 'string' !== typeof propertyValue) {
	    return 'exitCode must be string. Found: ' + propertyValue;
	}
	...
	propertyValue = getPropertyValue(i, 'assignmentId', ci); 
        ...
        if (inputCodesDb) {
            ...
            if (i[cfg.fields.exitCode]) {
                ...
                if (!code) {
                    str = 'ExitCode not found: ' + i[cfg.fields.exitCode];
                }
                else if (i[cfg.fields.exitCode] !== code.ExitCode) {
...
function validateResult(result, opts) {
    ...
    if (result[bonusField] < cfg.minBonus || result[bonusField] > cfg.maxBonus) {
