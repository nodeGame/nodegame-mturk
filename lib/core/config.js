...
function checkConfig(prg, cfg) {
    ...
    // Filter.
    if (cfg.filter && 'function' !== typeof cfg.filter) {
        logger.error('config.filter must be function or undefined. Found: ' +
                     cfg.filter);
        return;
    }

    // Create a copy of fields lowerCase, if
    // case-Insensitive is requested.
