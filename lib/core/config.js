...
function loadConfig(configPath) {
    ...
    // console.log(rootDir);
    // rootDir = J.resolveModuleDir('nodegame-mturk');
    // console.log(rootDir);
    ...
    if (configPath) {
        ...
        configPath = path.join(rootDir, 'conf', 'mturk.conf.js');
        config = require(configPath);
    }
    ...
    // Backward compatible.
    if (!config.accessKeyId && config.access) {
        config.accessKeyId = config.access;
        delete config.access;
        logger.warn('config.access is deprecated, use config.accessKeyId');
    }
    if ('string' !== typeof config.accessKeyId ||
        config.accessKeyId.trim() === '') {

        logger.error('config.accessKeyId must be a non-empty string. Found: ' +
                     config.accessKeyId);
        return;
    }
    
    if (!config.secretAccessKey && config.secret) {
        config.accessAccessKey = config.secret;
        delete config.secret;
        logger.warn('config.secret is deprecated, use config.secretAccessKey');
    }
    if ('string' !== typeof config.secretAccessKey ||
        config.secretAccessKey.trim() === '') {

        logger.error('config.secretAccessKey must be a non-empty string. ' +
                     'Found: ' + config.secretAccessKey);
        return;
    }
...
function checkConfig(prg, cfg) {
    ...
    if (cfg.nodegamePath) {
        cfg.nodeGamePath = cfg.nodegamePath;
        logger.warn('option nodegamePath should be nodeGamePath');
    }
    if (cfg.nodeGamePath) {
        if ('string' !== typeof cfg.nodeGamePath) {
            logger.error('option nodeGamePath must be string or undefined. ' +
                         'Found: ' + cfg.nodeGamePath);
            return;
        }

        cfg.nodeGamePath = path.resolve(cfg.rootDir, cfg.nodeGamePath);
    }
