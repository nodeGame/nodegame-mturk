...
function computeResultStat(item) {
    ...
    if (b) {
        stat.bonus.count++;
        stat.bonus.total += b;
        if ('NA' === typeof stat.bonus.max || b > stat.bonus.max) {
            stat.bonus.max = b;
        }
        if ('NA' === typeof stat.bonus.min || b < stat.bonus.min) {
            stat.bonus.min = b;
        }
...
function getCurrentStats(mod) {
    if (mod === 'bonus') return currentStats.result.bonus;
    if (mod === 'result') return currentStats.result.result;
    if (mod === 'qualification') return currentStats.result.qualification;
    return currentStats;
}
