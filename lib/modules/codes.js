...
function loadResults(args, cb) {
    ...
    logger.info('input codes file: ' + resultsFile);

    // Results File.
    if (args.resultsFile) {
        if ('string' !== typeof args.resultsFile ||
            args.resultsFile.trim() === '') {

            logger.error('--resultsFile is invalid. Found: ' +
                         args.resultsFile);
            return;
        }
        cfg.resultsFile = args.resultsFile
    }
