...
function getLastHITId(args, cb) {
    ...
    getLastHIT(function(err, hit) {
        if (err) {
            logger.error('an error occurred retrieving last HIT');
            if (cb) cb(err);
            return;
        }
...
function getHITs(args, cb) {
    ...
    api.req('ListHITs', {
        ...
        // Filter items with given query criteria.
        if (res.HITs && query) {
            res.HITs = res.HITs.filter((hit) => {
                if (hit.Title.lastIndexOf(query) !== -1) return hit;
            });
        }
        // console.log(res);
...
function selectQualificationType(args, cb) {
	...
	inquirer
	    .prompt(questions)
	    .then(function(answers) {
                // console.log(answers.QualificationType);
...
function _getQualificationTypes(cb, query) {
    ...
    api.req('ListQualificationTypes', opts, function(res) {
