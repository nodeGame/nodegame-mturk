...
function selectHITId(args, cb) {
	...
	let questions = {
		...
		d = d.toLocaleDateString() +
		    ' - ' + hit.HITId + ' ' + hit.Title + ' ($' + hit.Reward + ')';
		hit.name = hit.Title;
		return {
		    name: d,
		    value: hit
...
function selectQualificationType(args, cb) {
    _getQualificationType((err, quals) => {
	// console.log(hits);
	if (err) {
	    if (cb) cb();
	    return;
	...
	let questions = {
	    ...
	    message: 'Select an QualificationType',
	    choices: quals.map((q, idx) => {
		let d = new Date(0);
		d.setUTCSeconds(hit.CreationTime);
		d = d.toLocaleDateString() + ' ' + q.Title;
		return {
		    name: d,
		    value: q
		};
...
function getBonusList(args, cb) {
    ...
    api.req('ListBonusPayments', params, function(res) {
        if (params.show) {
            ...
            console.log();
            console.log(res.BonusPayments.length ?
			res.BonusPayments : ' - None');
            console.log();
        }
