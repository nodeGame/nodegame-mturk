...
function grantBonus(data, cb, args) {
	...
	err = 'missing Reason for bonus for WorkerId: ' + data.WorkerId;
    }
    else if ('string' !== typeof reason) {
        err = 'invalid Reason for bonus for WorkerId: ' + data.WorkerId + 
            '. Found: ' + reason;
    }
    if (err) {
        logger.error(err);
    ...
    if (!J.isNumber(bonus, 0)) {
        err = 'invalid or zero bonus for WorkerId: ' + data.WorkerId +
            '. Found: ' + bonus;
        logger.warn(err);
        if (cb) cb(err);
        return;
