...
function uploadResults(args, cb) {
    ...
    balance.showDiff({}, function(done) {
        // Do it!
        // TODO: measure time per request and adjust time interval.
        // In fact requests were being retried before the reply arrived.
        // So they succeded, but an error was also thrown.
        var counter = 0;
        resultsDb.each(function(result, args) {
            setTimeout(function() {
                var d = Date.now();
                uploadResult(result, function(err) {
                    var d1 = Date.now();
