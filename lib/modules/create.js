...
function createHIT(args, cb) {
    ...
    params.UniqueRequestToken += '';
    console.log('Unique Request Token: ' + params.UniqueRequestToken);

    // GO!
    api.req('CreateHIT', params, function(res) {
        logger.info('HIT Created: ' + res.HIT.HITId);
        logger.info('Preview (Need to be logged-in as worker):');
        let site = cfg.sandbox ? 'workersandbox' : 'worker';
        logger.info('https://' + site + '.mturk.com/mturk/preview?groupId=' +
                    res.HIT.HITGroupId);
        // Store a copy of hit.
        HITs[res.HIT.HITId] = res.HIT;
        if (cb) cb();
    }, function(err) {
        if (cb) cb(err);
    });
